...
# Preserved structural tool mappings
...
# Imported updated, bug-free math subroutines & schemas
...
from dunk_ai.tools.loan_clarity.validations import BaseLoanEngineSchema, AdvancedLoanEngineSchema
...
# --- Reusable Payload Schema Layouts ---
...
def calculate_flat_rate_emi(payload: LoanPayload):
    # Adapts to schema execution under the hood seamlessly
    schema_bridge = BaseLoanEngineSchema(
        loan_amount=payload.principal,
        interest_rate=payload.annual_rate,
        tenure_years=payload.tenure_years,
        monthly_income=1.0, # Placeholder fallback variable for pure math
        repayment_frequency=payload.repayment_frequency,
        interest_method="flat"
    )
    emi, total_interest, total_payment, number_of_payments = _handle_errors(
        flat_rate_from_schema, loan=schema_bridge
    )
    return {
        "emi": emi,
...
def calculate_reducing_balance_emi(payload: LoanPayload):
    ...
    emi, total_interest, total_payment, number_of_payments = _handle_errors(
        reducing_balance_from_schema, loan=schema_bridge
    )
    return {
        "emi": emi,
        "total_interest": total_interest,
...
def prepayment_impact(payload: PrepaymentRequest):
    # Map raw requests into validated schema profiles natively
    schema_bridge = AdvancedLoanEngineSchema(
        loan_amount=payload.principal,
        interest_rate=payload.annual_rate,
        tenure_years=payload.tenure_years,
...
def modify_emi_amount(payload: ModifyEmiRequest):
    ...
    result = _handle_errors(
        modify_emi,
        loan=schema_bridge,
        new_emi=payload.new_emi
    )
    return result


@router.post("/modify/tenure")
def modify_tenure_period(payload: ModifyTenureRequest):
    result = _handle_errors(
        modify_tenure,
        payload.principal,
        payload.annual_rate,
...
def compare_loan_options(payload: LoanComparisonRequest):
    if not payload.loan_options:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="At least one loan option is required.")
