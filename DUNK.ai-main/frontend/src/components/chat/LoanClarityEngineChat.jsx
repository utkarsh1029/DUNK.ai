...
// Upgraded standalone amount regex matcher
const extractNumber = (input, keywords) => {
  // 1. First Pass: Try to extract based on specific context words (your original logic)
  ...
  // 2. Fallback Second Pass: Catch naked values like "5 lakh" or "₹5,00,000" if no keywords hit
  const fallbackRegex = /(?:₹|rs\.?|rupees)?\s*([\d,.]+)\s*(l|lac|lakh|lakhs|cr|crore|crores)\b/i;
  const fallbackMatch = input.match(fallbackRegex);
  if (fallbackMatch) {
    return normalizeAmount(fallbackMatch[1], fallbackMatch[2]);
  }
...
const DEFAULT_PROFILE = {
  ...
  reduce_emi: true,
  monthly_income: 100000, // Safe backend schema baseline fallback default
...
const LoanClarityEngineChat = ({ sidebarOpen, setSidebarOpen, user }) => {
  ...
  const baseLoanParams = (userInput, overrides = {}) => {
    ...
    return {
      ...
      monthly_income: overrides.monthly_income || 100000, // Bound fallback check
      ...overrides
    };
  };
...
const generateResponse = async (userInput) => {
    ...
    // 2. Scrape the fresh inputs from the current message
    const freshPayload = {};
    const principalVal = extractNumber(userInput, ['principal', 'loan', 'amount']) || 
                         // Fallback matching naked numbers like "5 lakh" without keywords
                         userInput.match(/(?:₹|rs\.?|rupees)?\s*([\d,.]+)\s*(l|lac|lakh|lakhs|cr|crore|crores)\b/i) &&
                         normalizeAmount(userInput.match(/(?:₹|rs\.?|rupees)?\s*([\d,.]+)\s*(l|lac|lakh|lakhs|cr|crore|crores)\b/i)[1], userInput.match(/(?:₹|rs\.?|rupees)?\s*([\d,.]+)\s*(l|lac|lakh|lakhs|cr|crore|crores)\b/i)[2]);
    
    if (principalVal) freshPayload.principal = principalVal;
    
    const rateVal = extractRate(userInput);
    if (rateVal) freshPayload.annual_rate = rateVal;
    
    const yearsVal = extractYears(userInput);
    if (yearsVal) freshPayload.tenure_years = yearsVal;
    
    const paymentsVal = extractPaymentsMade(userInput);
    if (paymentsVal) freshPayload.payments_made = paymentsVal;
    
    const prepayVal = extractNumber(userInput, ['prepayment', 'lump sum']);
    if (prepayVal) freshPayload.prepayment_amount = prepayVal;
    
    const newEmiVal = extractNumber(userInput, ['new emi']);
    if (newEmiVal) freshPayload.new_emi = newEmiVal;

    // 3. Context Management Logic:
    // If we are already in a middle of a conversational loop (pendingIntent exists), 
    // we build on top of the pending data.
    // If it's a completely brand new chat message, we start fresh so old query variables don't ghost over.
    let payload = {};
    if (pendingIntent) {
      payload = { ...pendingPayload.data, ...freshPayload };
    } else {
      payload = { ...freshPayload }; 
    }

    // 4. Identify fields that are strictly missing
    const missingFields = [...fields].filter((field) => payload[field] === undefined);
    
    if (missingFields.length > 0) {
  // Lock the engine tracking flags until the user feeds the missing data properties
  setPendingIntent(intent);
  setPendingPayload({ intent, data: payload });
  
  const readableFields = missingFields.map(f => f.replace(/_/g, ' ')).join(', ');
  
  // 💻 FIXED: Removed the ** symbols so it prints clean plain text
  return `Almost there! To calculate your ${intent.replace(/_/g, ' ')}, please provide the missing input for: ${readableFields}.`;
}

    // --- Beyond this point, all inputs are complete. Clear the loop states ---
    setPendingIntent(null);
    setPendingPayload({});
    
    const rememberedFields = [...fields].reduce((acc, field) => {
      if (field !== 'loan_options' && payload[field] !== undefined) {
    ...
    // 5. Fire your API requests as usual...
    if (intent === 'schedule') {
    ...
    // ... (Keep the rest of your endpoint if statements exactly the same) ...
